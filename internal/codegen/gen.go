...
var (
	...
	debugOut    = flag.String("debug-out", "../../debug/generated.go", "")
)
...
func main() {
...

	{
		...
		fmt.Fprintf(&buf, "	%q\n", "github.com/hexon/fastmsgpack/internal")
		fmt.Fprintf(&buf, ")\n")
		generate(&buf, "debug", "debugValue")

		formatted, err := format.Source(buf.Bytes())
		if err != nil {
			formatted = buf.Bytes()
		}
		if err := os.WriteFile(*debugOut, formatted, 0644); err != nil {
			log.Fatal(err)
		}
	}
}
...
func generateDecodeType(w *bytes.Buffer, retType, thisFunc string, guaranteedLength int, t MsgpackType) {
	...
	case "debug":
		switch t.DataType {
		case "array":
			fmt.Fprintf(w, "		return p.%s_array(data, %s, %s)\n", lcfirst(thisFunc), lencalc, val)
		case "map":
			fmt.Fprintf(w, "		return p.%s_map(data, %s, %s)\n", lcfirst(thisFunc), lencalc, val)
		case "ext":
			fmt.Fprintf(w, "		return %s, p.%s_ext(data[:%d], %s, int8(data[%d]))\n", lencalc, lcfirst(thisFunc), genericz.Ternary(strings.HasPrefix(t.Name, "fixext"), t.DataStart, minLen), val, t.ExtTypeAt)
		case "[]byte":
			fmt.Fprintf(w, "		return %s, p.appendBytes(data[:%d], %s)\n", lencalc, minLen, val)
		default:
